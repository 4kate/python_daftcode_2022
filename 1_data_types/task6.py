def lazy_scribe(sources: list):
    ...
    result: str = ''
    list_item = 0
    while list_item < len(get_max_str(sources)):
        for element in sources:
            if list_item < len(element):
                if len(result) > 0:
                    if result[len(result)-1] == element[list_item]:
                        reps = 2
                        if result[len(result)-2].isnumeric():
                            reps = reps + 1  
                            result = result[:-2] + str(reps) + element[list_item]
                        else:
                            result = result[:-1] + str(reps) + element[list_item]
                    else:
                        result = result + element[list_item]
                else:
                    result = result + element[list_item]
